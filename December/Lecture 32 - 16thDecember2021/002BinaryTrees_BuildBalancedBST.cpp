...
node* buildBBST(int* in, int s, int e) {
	...
	// base case
	if(s > e) {
		// build a balanced BST which is empty 
		return NULL;
	}
	...
	// build balanced leftSubtree from sorted subArray in[s:m-1]  -- ask your friend
	root->left = buildBBST(in, s, m-1);

	// build balanced rightSubtree from sorted subArray in[m+1:e] -- ask your friend
