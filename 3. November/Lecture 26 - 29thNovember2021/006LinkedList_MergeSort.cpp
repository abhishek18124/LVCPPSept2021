...
node* mergeSort(node* head1) {
    ...
    if(!head1 || !head1->next) {
        // given linkedList is either empty or
        // contains a single node therefore it
        // is already sorted
        return head1;
    }

    // 1.1 compute the midPoint
    node* midPoint = getMidPoint(head1);

    // 1.2 Divide the linked list around the midPoint
    node* head2 = midPoint->next;
    midPoint->next = NULL;

    // 2. Sort the subLinkedList recursively
