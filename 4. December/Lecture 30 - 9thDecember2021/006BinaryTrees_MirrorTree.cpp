...
node* mirror(node* root) {
	...
	// base case
	if(root == NULL) {
		// you're trying to mirror an empty
		return NULL;
	}

	// recursive case

	// 1. mirror the left subtree -- ask your friend
	root->left = mirror(root->left);

	// 2. mirror the right subtree -- ask your friend
	root->right= mirror(root->right);

	// 3. swap root's left child with its right child
